...
logging.basicConfig(level=logging.INFO, filename='logfile_XGBoost.log', filemode='a',
...
pipe = Pipeline([
    ...
    ("xgb", XGBClassifier(use_label_encoder=False, eval_metric='logloss', random_state=114514))
])

param_grid = {
    'transformer__op': op_list,
    'transformer__k': gap_list,
    'xgb__learning_rate': learning_rate_list,
    'xgb__n_estimators': n_estimators_list
}
...
for op in op_list:
    for gap in gap_list:
        ...
        grid_search = GridSearchCV(pipe, param_grid, scoring=my_scorer, cv=RepeatedKFold(n_splits=5, n_repeats=1, random_state=114514), n_jobs=8, verbose=3)
